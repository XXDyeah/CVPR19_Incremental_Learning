    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(trainloader):
            if len(data) == 4:
                ...
                indices = indices.to(device)
                flags = torch.zeros(inputs.size(0), dtype=torch.long, device=device)
            else:
                inputs, targets = data
                indices = torch.arange(inputs.size(0), device=device, dtype=torch.long)
                flags = torch.zeros(inputs.size(0), dtype=torch.long, device=device)
            inputs, targets = inputs.to(device), targets.to(device)
            tg_optimizer.zero_grad()

            # Generate counterfactual samples if requested
            ...
            if iteration == start_iteration:
                ...
                loss1 = nn.KLDivLoss()(F.log_softmax(real_logits[:,:num_old_classes]/T, dim=1), \
